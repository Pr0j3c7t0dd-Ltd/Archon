...
async function evaluateLoopBashCondition(options: LoopBashConditionOptions): Promise<boolean> {
  ...
  try {
    await runSubprocess(options.execContext, resolveBashPath(), ['-c', substitutedBash], {
      cwd: options.cwd,
    ...
    const bashError = error as NodeJS.ErrnoException;
    if (
      bashError.code === 'ENOENT' ||
      bashError.code === 'EACCES' ||
      bashError.code === 'ENOTDIR'
    ) {
      getLog().warn({ err: bashError }, 'loop_node.bash_condition_exec_error');
    } else if (typeof bashError.code !== 'number') {
      getLog().warn({ err: bashError }, 'loop_node.bash_condition_unexpected_error');
    }
    return false;
  }
}
  ...
  for (let i = startIteration; i <= loop.max_iterations; i++) {
    ...
    if (iterationIdleTimedOut && !iterationExternallyCompleted && iterationUntilBash) {
      try {
        ...
        getLog().warn(
          { err: error as Error, nodeId: node.id, iteration: i },
          'loop_node.iteration_until_bash_after_idle_timeout_error'
        );
      }
    }

    // Notify on idle timeout or deterministic per-iteration completion.
    if (iterationIdleTimedOut && !iterationExternallyCompleted) {
      await safeSendMessage(
        platform,
        conversationId,
        `Loop node '${node.id}' iteration ${String(i)} hit idle timeout before deterministic completion (no output for ${String((node.idle_timeout ?? STEP_IDLE_TIMEOUT_MS) / 60000)} min)`,
        msgContext
      );
    ...
    if (iterationIdleTimedOut && !iterationExternallyCompleted) {
      ...
      return failLoopNode(`Loop iteration ${i} failed: ${idleError}`, {
        costUsd: loopTotalCostUsd,
        ...(loopTotalTokens !== undefined ? { tokens: loopTotalTokens } : {}),
        loopIterations: i,
        data: { iteration: i },
    ...
    // budget producing nothing. A successful iteration_until_bash check is
    // the only valid zero-output completion path.
